    ...
    private class MonitorBehaviour extends TickerBehaviour {
        ...
        private void finishSystem() {
            try {
                ...
                System.out.println("[Supervisor] Generating final JSON files...");
                ProfesorHorarioJSON.getInstance().generarArchivoJSON();
                SalaHorarioJSON.getInstance().generarArchivoJSON();
                System.out.println("[Supervisor] System shutdown complete.");
                myAgent.doDelete();
            } catch (Exception e) {
                System.err.println("[Supervisor] Error finishing system: " + e.getMessage());
                e.printStackTrace();
            }
        }
    }
